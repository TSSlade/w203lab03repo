...
model1_trans <- with(crime_na, lm(crmrte ~ log(polpc) + log(prbarr) + prbconv + prbpris^2 + avgsen))
...
While our first model showed promise there are several other factors which might be correlated with these explanatory variables; this would lead to multicollinearity issues. \textbf{\color{red}{To-Do: add a sentence or two about why that's a problem.}} In order to control for this we created a second model which includes variables we believe to be highly correlated with our three key variables.

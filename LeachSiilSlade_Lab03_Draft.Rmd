...
```{r data_import, results = "hide", message=FALSE, warning=FALSE}
...
Our first step was to evaluate if any variables had signficant skew in their distributions by checking whether they generally conformed to a normal distribution using R's qqplot.  While this is not necessarily a reason to transform a variable, it can help us identify variables of interest.
...
\texttt{density} & \textit{people per sq. mile} & $\sqrt(density)$ \\
\texttt{taxpc} & \textit{tax revenue per capita} & $\log(taxpc)$ \\
\texttt{west} & \textit{Dummy: source county of data is in Western NC} & - \\
\texttt{central} & \textit{Dummy: source county of data is in Central NC} & - \\
\texttt{urban} & \textit{Dummy: source county of data is urban} & - \\
\texttt{pctmin80} & \textit{percent minority in 1980}\\
\texttt{wcon} & \textit{wages in the construction industry}\\
\texttt{wtuc} & \textit{wages in the transportation, utilities, and communication industries} & - \\
\texttt{wtrd} & \textit{wages in the construction industry} & - \\
\texttt{wfir} & \textit{wages in the finance, insurance, real estate industries} & - \\
\texttt{wser} & \textit{wages in the service industry} & - \\
\texttt{wmfg} & \textit{wages in the manufacturing industry} & - \\
\texttt{wfed} & \textit{wages among federal employees} & - \\
\texttt{wsta} & \textit{wages among state employees} & - \\
\texttt{wloc} & \textit{wages among local government employees} & - \\
\texttt{mix} & \textit{mix of offenses; face-to-face v others} & - \\
\texttt{pctymle} & \textit{percent young male} & $\log(pctymle)$ \\
\hline
\end{tabular}
\end{center}

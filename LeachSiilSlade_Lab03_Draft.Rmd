...
Additionally, there have been media reports that income inequality is a factor in crime.  In an attempt to proxy this we created a fourth wage variable "income_inequality_proxy" which is the difference between the "Physical Labor Wage" and the "Industry Wage"
...
To begin our model building process we wanted to understand how each explanatory variable looked in isolation.  This approach can help illuminate variables of interest, as well as ones which can be reasonably excluded from our first model.  It's important to note that a low R squared value alone doesn't mean the variable isn't of interest, as that explanatory power might be independent of other variables; however, if the R squared value is extroardinarily low, then we can confidently exlucde it from our first model.
...
lm_govtwg <- with(crime_na, lm(crmrte ~ govt_wg))
lm_physicalwg <- with(crime_na, lm(crmrte ~ physical_wg))
lm_industrywg <- with(crime_na, lm(crmrte ~ industry_wg))
...
```{r stargazer_simple_ols, results="asis"}
...
There are several very important findings we can deduce from our simple linear regressions.  First, there are several variables which have negligible explanatory power and can be excluded from our first model which is focused on parsimony.  We categorize these into three categories:

Enforecement:

  1) Probability Prison (prbpris) has an R squared value of .002 suggesting that an increase in incarcaration rates is not an effective deterrent of crime.  
  
  2) Average Sentence (avgsen) has an R squared value of .0004, which implies that average sentence length has essentially no predictive capability.
  
  The fact that these two variables have little explanaory power has far reaching consequences for our policy reccomendations.  If we accept these two models as causal, it would suggest that neither increasing prison sentences nor increasing sentencing rates has an influence on the crime rate.  In fact, the coefficients for both variables are positive, indicating that as   Therefore, any legislation which attempts to reduce crime through these mechanisms is likely to fail.
  
Environmental:

  1) Income Inequality (income_inequality_proxy) has an R squared value of .003.  This suggests to us that at least our proxy for income inequality is not predictive of crime rates in NC during this time period.
  

Demographic:

  1) Percent Minority in year 1980 (pctmin80) has an R squared value of .033 suggesting that in isolation crime rates are not linearly associated with minority populations.

Note: While Police per capita has a low R squared value we will continue to include it as it is crucial to understand this variable in order to make informed policy decisions.
  
From our remaining variables there are two pairs which may be highly correlated.  We looked at probability of arrest, and probability of conviction, as well as Government wage and Industry wage.  

``` {r model 1 pair comparison}
lm_wagecompare <- with(crime_na, lm(crmrte ~ govt_wg+industry_wg))
lm_enforcement <- with(crime_na, lm(crmrte ~ prbarr+prbconv))

summary(lm_wagecompare)
summary(lm_enforcement)
```

From the above we learn that when including both our govt_wg and industry_wg variables, the coefficient of industry_wg decreases significantly, while there is a much smaller change in the govt_wg coefficient.  Because of this, we will only include govt_wg in our first model.  

When including both prbarr and prbconv our coefficients remain largely unchanged, and our adjusted R squared increases significantly.  Therefore we will continue to use both variables.

After this initial round of analysis we decided to create our first model which uses Police per capaita, percent male, density, log of tax revenue per capita, probability of arrest, probability conviction, and government wage.

``` {r model 1}
model1 <- with(crime_na, lm(crmrte ~ polpc + pctymle + density+ log(taxpc, base = 10)+prbarr+prbconv+govt_wg))
summary(model1)
```
...
lm_modwages <- with(crime_na, lm(crmrte ~ wcon + wtuc + wtrd +wfir +wser + wmfg + wfed + wsta + wloc))

...
```{r}
crime_na %>% 
  gather(wcon, wtuc, wtrd, wfir, wser, wmfg, wfed, wsta, wloc,  key = 'work_type', value = 'weekly_wage') %>%
...
test <- crime_na %>% 
  gather(wcon, wtuc, wtrd, wfir, wser, wmfg, wfed, wsta, wloc,  key = 'work_type', value = 'weekly_wage') %>%
  select(county, work_type, weekly_wage) %>%
  group_by(work_type) %>%
  summarise(ave_weekly_wage = mean(weekly_wage)) %>%
  arrange(desc(ave_weekly_wage)) %>%
  select(1)
```

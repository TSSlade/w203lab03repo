...
In the previous section we discussed the significance of the "service wage" variable.  A single wage variable might be signififant just by chance, so one way to gauge if wage levels in general are predictive would be to calculate an F statistic for a join hypothesis test.  This will help us determine if wage levels are jointly significant.  First, we will look at the distributions of wage variables to ensure nothing is too abnormal:
...
Next, we will calculate the F statistic for our joint hypothesis (All wage coefficients equal to zero):
...
Despite the promising results from our three models it is difficult to ascribe causality to the variables of interest.  One issue with causal inference in general is ommitted variable bias, which can invalidate our ability to assume each explanatory variable is uncorrelated with the error term.  While we may not always be able to include a variable in our model, there are times when we can calculate the direction of the bias.

As a quick refresher to the reader, the Bias in the coefficient of X1 as a result of an ommitted variable X2 is equal to the product of the omitted variable's coefficient if it were to be included in the population model (Beta 2), and the coefficient belonging to X1 when X2 is regressed on X1.

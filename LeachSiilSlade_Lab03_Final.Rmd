...
make_scatters <- function(df, var_list, y, trans) {
  var_list_new <- c()
  for (var in var_list){
    var_list_new <- append(var_list_new, var)
    
    if(!missing(trans)) {
    var_list_new <- append(var_list_new, str_glue('{trans}({var})'))
    }
  }
  for (v in var_list_new){
...
With regards to interpretability, the interpretation of a log transformation is that a percentage change in the independent variable (for small changes <20%) will illict a constant change in the dependent variable. A square root transformation has a slope which decays asymptotically to zero, so the effect dampens as the values become larger.  In the case of `density`, this represents that increased density in cities may not have as large of an effect. Given the improvement in linearity and normality we will use these transformations moving forward. 
...
While we believe these variables are sufficiently distinct to warrant their own model, we believe they may exhibit some collinearity with the predictors we included in \textbf{Model 1}. Specifically, we suspect `taxpc` and `density` may be collinear with `polpc`; `pctymle` and `pctmin80` may be collinear with `prbarr`; and government wages (`wfed`, `wsta`, `wloc`) may be collinear with `prbconv`.^[The expected collinearity of government wages with `prbconv` is premised on a theorized relationship between `prbconv` as an outcome variable and the quality or effectiveness of police investigators, court-appointed advocates, judges, and other government officials \textit{as proxied} loosely \textit{by wage}.] We theorize that both `prbpris` and `avgsen` may be collinear with some underlying omitted variable (for instance, local sentencing guidelines), but lack an alternative variable that might serve as a reasonable proxy. We will conduct checks for collinearity when analyzing our models.
...
2. \textit{Random Sampling}: The data on which the model is based represents a random sample from the population and the generative mechanism produces i.i.d data.
...
4. \textit{Zero-conditional mean / Exogeneity}: 
    $$E[u|x_i] = 0;\text{ }i=1...k$$
    $$Cov(x_i,u)=0 ;\text{ } i=1...k$$
    $$E[u]=0$$

5. \textit{Homoskedasticity}: Residuals display equal variance for all $X_i$.
    $$Var(u|x_i)=\sigma^2]; \text{ } i=1...k$$

6. \textit{Normality of Errors}: Residuals will be normally distributed.

We cannot empirically test \textbf{CLM2}, but we will assume it is met as our "universe" is counties in N.C and we have data on the entire population.
...
The \textit{p}-value associated with our heteroskedasticity-robust F-test is highly significant, indicating that our model is more predictive than a simple horizontal line at a y-intercept would be. The adjusted $R^2$ of our model is `r round(mod1_summ$adj.r.squared, 3)`, which is reasonably high given the complexity of crime.
...
shapiro.test(mod2_b$residuals) %>% tidy() %>% kable()
```
...
``` {r joint_wage_significance, echo = TRUE, results = TRUE}
#Conduct join hypothesis F-test to determine if wages are significant when considered collectively
...
\textbf{Model 2} yielded greater predictive value than \textbf{Model 1} by including a few extra covariates. \textbf{Model 3} throws in every available variable in the dataset.
...
mod3 <- with(crime_na, lm(crmrte ~ log(polpc) + log(prbarr) + log(prbconv) + prbpris +
                            avgsen + log(taxpc) + sqrt(density) + pctymle +
                            pctmin80 + west + central + urban + mix
                          +wcon + wtuc + wtrd + wfir + wser + wmfg + wfed + wsta + wloc))
...
Similar to the other models, the mean of the residuals appears to be centered on 0. Based on the Normal QQ plot our residuals appear to be fairly normal, except at the extremes of our model where there are fewer data points. Again, we apply the Shapiro-Wilke test for normality to our \textbf{Model 3} residuals.

```{r mod3_normality_residuals, echo = FALSE}
shapiro.test(mod3$residuals)
```

Since the test results in a p value of less than 0.05 we reject the null hypothesis that the underlying distribution of the residuals in normal. To compensate, we will use heteroskedasticity robust statistical versions of the standard error, F and other values.

```{r mod3_het_corrections}
...
``` {r mod3_b_coeff_output, echo = FALSE, results = "asis"}
stargazer(mod3, header = FALSE, type = "latex",
          se = mod3_robse, p = mod3_pvals,
          report="vc*s", star.cutoffs = c(0.05, 0.01, 0.001),
          title = "Model 3: The Kitchen Sink", single.row = TRUE,
          omit.stat = "f",
          add.lines = list(c("F Statistic", mod3_wF), c("Pr(>F)", mod3_wp)),
          notes = "p-values based on Wald test; robust standard errors in parentheses")
```

\pagebreak
\textbf{Comments on Model 3: The Kitchen Sink}
...
Below is a comparison table for our three initial models. The table reports key statistics related to each model, including the coefficients for each predictor, the $R^2$ and adjusted $R^2$, and the \textit{Akaike information criterion}, or AIC. 
...
As a quick refresher to the reader, the Bias in the coefficient of X1 as a result of an omitted variable X2 is equal to the product of the omitted variable's coefficient if it were to be included in the population model (Beta 2), and the coefficient belonging to X1 when X2 is regressed on X1 (delta).

True Model:
$$y=\beta_0+\beta_1 x_1+\beta_2x_2 + u$$
...
make_scatters(crime_na, var_list = colnames(crime_na), "crmrte")
```
...
make_qqs(df = 'crime_na', var_list = c('prbarr', 'prbconv', 'polpc', 'taxpc'), trans = 'log')
```

```{r skewnessgraphs_appendix2, out.width = '50%', results='hide', fig.keep='all'}
make_qqs(df = 'crime_na', var_list = c('density'), trans = 'sqrt')
```

...
The variables in this theme are particularly valuable because they provide a solid foundation for policy development. If the size of the police force is associated with changes in the crime rate, it can be scaled up or scaled down appropriately. If the probability of arrest makes a difference, the county could invest in improving community-police relationships with the goal \textbf{\color{red}{of improving investigations and... <- TS: sort of lost my train of thought here.}} If the probability of conviction is important, funds could be invested in further training the police and District Attorneys on the collection and presentation of evidence. If the probability of incarceration and average sentence are important, legislators could advocate for changes to sentencing guidelines.
...
\begin{align}
    \texttt{crmrte} = \beta_0 &+ \beta_1\log(\texttt{polpc}) + \beta_2\texttt{prbarr} + \beta_3\texttt{prbconv} + \beta_4\texttt{prbpris}^2 + \beta_5\texttt{avgsen} \tag{Model 1} \nonumber \\
    &+ \beta_6\log(\texttt{taxpc}) + \beta_7\texttt{density} + \beta_8\texttt{pctymle} + \beta_9 \texttt{pctmin80} + \beta_{10}\texttt{} \nonumber \\
    &+ \beta_{11}\texttt{} + \beta_{12}\texttt{} + \beta_{13}\texttt{} + \beta_{14}\texttt{} \nonumber \\
    &+ \beta_{15}\texttt{} + \beta_{16}\texttt{} + \beta_{17}\texttt{} + \beta_{18}\texttt{} + u
\end{align}

\textbf{\color{red}{$\downarrow$ TS: This section still needs to find a new home. I just haven't found it yet.}}
...
\begin{align}
    \texttt{crmrte} = \beta_0 &+ \beta_1\log(\texttt{polpc}) + \beta_2\texttt{prbarr} + \beta_3\texttt{prbconv} + \beta_4\texttt{prbpris}^2 + \beta_5\texttt{avgsen} \tag{Model 1} \nonumber \\
    &+ \beta_6\log(\texttt{taxpc}) \beta_7\texttt{density} + \beta_8\texttt{pctymle} + \beta_9 \texttt{pctmin80} + \beta_{10} \nonumber \\
    &+ \beta_{11} + \beta_{12} + \beta_{13} + \beta_{14} \nonumber \\
    &+ \beta_{15} + \beta_{16} + \beta_{17} + \beta_{18} \tag{Model 2} \nonumber \\ 
...
Our adjusted R squared value for this model is `r round(summary(model3_trans)$adj.r.squared, 3)` which represents a negligible improvement over our previous model.
